...
void wifi_onboarding_gatt_init()
{
    ...
    gatt_status =  wiced_bt_gatt_db_init( wifi_onboarding_gatt_database, wifi_onboarding_gatt_database_size );
 ...
 */
wiced_bt_gatt_status_t wifi_onboarding_gatts_req_read_handler( uint16_t conn_id, wiced_bt_gatt_read_t * p_read_data )
{
    attribute_t *puAttribute;
    int          attr_len_to_copy;

    if ( ( puAttribute = wifi_onboarding_get_attribute(p_read_data->handle) ) == NULL)
    {
 ...
 */
wiced_bt_gatt_status_t wifi_onboarding_gatts_req_write_handler( uint16_t conn_id, wiced_bt_gatt_write_t * p_data )
{
...
wiced_bt_gatt_status_t wifi_onboarding_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_req )
{
    ...
    WICED_BT_TRACE( "wifi_onboarding_gatts_req_cb. conn %d, type %d\n", p_req->conn_id, p_req->request_type );

    switch ( p_req->request_type )
    {
    case GATTS_REQ_TYPE_READ:
        result = wifi_onboarding_gatts_req_read_handler( p_req->conn_id, &(p_req->data.read_req) );
        break;

    case GATTS_REQ_TYPE_WRITE:
        result = wifi_onboarding_gatts_req_write_handler( p_req->conn_id, &(p_req->data.write_req) );
        break;

    case GATTS_REQ_TYPE_WRITE_EXEC:
        result = wifi_onboarding_gatts_req_write_exec_handler( p_req->conn_id, p_req->data.exec_write );
        break;

    case GATTS_REQ_TYPE_MTU:
        result = wifi_onboarding_gatts_req_mtu_handler( p_req->conn_id, p_req->data.mtu );
        break;

    case GATTS_REQ_TYPE_CONF:
        result = wifi_onboarding_gatts_req_conf_handler( p_req->conn_id, p_req->data.handle );
        break;
...
{
    uint8_t *scan_data=p_buffer;
    uint16_t byte_no = len -2;
    uint16_t handle = 0;

    STREAM_TO_UINT16(handle, scan_data);
    if (conn_id != 0)
    {
        wiced_bt_gatt_send_notification( conn_id, handle, byte_no, scan_data );
    }
    return WICED_BT_GATT_SUCCESS;
}
